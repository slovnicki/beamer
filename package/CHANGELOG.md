...
- **BREAKING:** `BeamGuard` `beamTo` receives the origin `BeamLocation` alongside `BuildContext`.
- **BREAKING:** `BeamGuard` `beamToNamed` is now a function that receives the origin `BeamLocation` and return a `String`.
