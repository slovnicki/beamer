...
- **BREAKING:** `BeamGuard` `beamTo` receives the current `BeamLocation` alongside `BuildContext`.
- **BREAKING:** `BeamGuard` `beamToNamed` is now a function that receives a `BeamLocation` and return a `String`.
