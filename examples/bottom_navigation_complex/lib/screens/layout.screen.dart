...
class _LayoutScreenState extends State<LayoutScreen> {
  ...
  late List<Beamer> children = List.generate(
      routes.length,
      (index) => Beamer(
            backButtonDispatcher: BeamerBackButtonDispatcher(delegate: routerDelegates[index]),
  ...
  void _updateCurrentIndex(int index) {
    ...
    if (currentIndex != index) {
      ...
      if (currentIndex != -1) {
        WidgetsBinding.instance.addPostFrameCallback((_) => children[index].routerDelegate.update(rebuild: false));
      }
